...
#         samples = self.decode(z)  # 经过解码过程，得到生成样本Y
#         return samples  # 返回生成样本Y
class VAE(nn.Module):
...
if __name__ == "__main__":
    ...
    sample_channel = 1 
    latent_dim = 128
    sample_samples_dir = "sample_samples"
    train_dataset_dir = "../dataset/mnist" 

    os.makedirs(sample_samples_dir, exist_ok=True)  # 创建生成样本示例存放路径
    ...
    # img_size = (sample_channel, sample_size, sample_size)  # 输入样本形状(1,32,32)
    ...
    #     dataset=datasets(root=train_dataset_dir, 
    #                            train=True, 
    #                            transform=transform,
    #                            download=True),
    ...
    for epoch in range(total_epochs):  # 循环epoch
        ...
        # vae.eval()  # 模型开始验证
        # sample_samples = vae.sample(25, cuda)  # 获得25个生成样本
        # save_sample(sample_samples.data, "%s/ep%d.png" % (sample_samples_dir, (epoch + 1)), nrow=5,
